document.addEventListener('DOMContentLoaded',function(){
    ...
    // Story lines for text AND voice
    const storyLines=[
    ...
    function typeLine(){
        ...
        if(currentLine>=storyLines.length){setTimeout(endIntro,2000);return;}
        const line=storyLines[currentLine];
        if(line.text===""){narrationText.innerHTML="";currentLine++;setTimeout(typeLine,line.delay);return;}
        narrationText.innerHTML="";narrationContainer.classList.add('visible');
        let charIndex=0;
        function typeChar(){
            if(introSkipped) return;
    ...
    function endIntro(){
        if(introSkipped) return;introSkipped=true;
        if(synth) synth.cancel();
        intro.classList.add('hidden');mainWebsite.classList.remove('hidden');
        setTimeout(typeHeroSubtitle,1000);initParticles();
    }
    ...
    function typeHeroSubtitle(){
        ...
        let charIndex=0;heroTypewriter.textContent="";
        function typeChar(){if(charIndex<text.length){heroTypewriter.textContent+=text.charAt(charIndex);charIndex++;setTimeout(typeChar,50);}else{setTimeout(function(){heroTextIndex=(heroTextIndex+1)%heroTexts.length;typeHeroSubtitle();},3000);}}
        typeChar();
    }
    ...
    window.addEventListener('scroll',function(){
        if(window.pageYOffset>100){navbar.style.background='rgba(5,5,5,0.95)';navbar.style.borderBottomColor='rgba(0,255,136,0.2)';}else{navbar.style.background='rgba(5,5,5,0.8)';navbar.style.borderBottomColor='rgba(0,255,136,0.1)';}
    });
    
    // SMOOTH SCROLL
    document.querySelectorAll('a[href^="#"]').forEach(function(a){a.addEventListener('click',function(e){e.preventDefault();const t=document.querySelector(this.getAttribute('href'));if(t)window.scrollTo({top:t.offsetTop-80,behavior:'smooth'});});});
    
    // SCROLL REVEAL
    const revealElements=document.querySelectorAll('.reveal');
    const revealObserver=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting){const delay=entry.target.dataset.delay||0;setTimeout(function(){entry.target.classList.add('active');},delay);revealObserver.unobserve(entry.target);}});},{threshold:0.1,rootMargin:'0px 0px -50px 0px'});
    ...
    thumbs.forEach(function(thumb){thumb.addEventListener('click',function(){thumbs.forEach(function(t){t.classList.remove('active');});thumb.classList.add('active');mainImg.style.opacity='0';setTimeout(function(){mainImg.src=thumb.querySelector('img').src;captionTitle.textContent=thumb.dataset.title;captionDesc.textContent=thumb.dataset.desc;captionTag.textContent=thumb.dataset.file;mainImg.style.opacity='1';},300);});});
    
    // COPY TO CLIPBOARD
    document.querySelectorAll('.pay-value').forEach(function(el){el.addEventListener('click',function(){const text=el.dataset.copy;navigator.clipboard.writeText(text).then(function(){const original=el.innerHTML;el.innerHTML=text+' <i class="fas fa-check"></i>';el.style.color='#00ff88';setTimeout(function(){el.innerHTML=original;el.style.color='';},2000);});});});
